#!/bin/bash
##############################################################################
# Universal PXE Boot Order Enforcement Script v5.0
# Compatible with: RHEL, CentOS, Rocky, AlmaLinux, Fedora, Debian, Ubuntu,
#                  openSUSE, SLES, Arch, Manjaro, and more
# 
# Automatically detects distro and uses appropriate package manager
# Ensures PXE/Network boot remains first in boot order
##############################################################################
...
log "Starting Universal PXE Boot Order Enforcement Setup v5.0..."
...
if [ ! -d /sys/firmware/efi ]; then
    ...
    warn "This script only works on UEFI systems, not legacy BIOS."
    exit 0
fi
...
detect_distro() {
    ...
    info "Detected OS: ${DISTRO_NAME} (${DISTRO_ID})"
    log "Distribution: ${DISTRO_NAME} ${DISTRO_VERSION}"
}

# Install efibootmgr using appropriate package manager
install_efibootmgr() {
    if command -v efibootmgr &> /dev/null; then
        log "efibootmgr already installed"
        return 0
    fi
    ...
    case "${DISTRO_ID}" in
        rhel|centos|rocky|almalinux|fedora|ol)
            # RHEL-based: use dnf or yum
            if command -v dnf &> /dev/null; then
                dnf install -y efibootmgr
            elif command -v yum &> /dev/null; then
                yum install -y efibootmgr
            else
                error "No package manager found (expected dnf or yum)"
                return 1
            fi
            ;;
        ...
        opensuse*|sles|suse)
            ...
            zypper --non-interactive install efibootmgr
            ;;
        ...
        *)
            ...
            if echo "${DISTRO_ID_LIKE}" | grep -q "rhel\|fedora"; then
                if command -v dnf &> /dev/null; then
                    dnf install -y efibootmgr
                elif command -v yum &> /dev/null; then
                    yum install -y efibootmgr
                ...
                error "Please install efibootmgr manually and re-run this script"
                return 1
            fi
            ;;
    esac
    ...
    if command -v efibootmgr &> /dev/null; then
        log "efibootmgr installed successfully"
        return 0
    else
        error "Failed to install efibootmgr"
        return 1
    fi
}
...
# Install efibootmgr
if ! install_efibootmgr; then
    error "Cannot continue without efibootmgr"
    exit 1
fi
...
# Create the main script (universal - works on all distros)
cat > "${SCRIPT_PATH}" << 'EOFSCRIPT'
#!/bin/bash
##############################################################################
# Boot Order Fix Script v5.0 - Universal
# Ensures PXE/Network boot entries are first, OS boot entry is second
# Compatible with all Linux distributions using UEFI and systemd
##############################################################################
...
log_message() {
    ...
    logger -t boot-order-fix "$1" 2>/dev/null || true
}
...
# Find ALL PXE/Network boot entries - Match exactly 4 hex digits
...
Documentation=https://github.com/Dristen/pxe_booter
...
log ""
log "================================================================"
log "  Installation Complete - Universal PXE Boot Order Fix v5.0"
log "================================================================"
log ""
